...
public class App {
    ...
    public static void filter(HashMap<String, Employee> employees, String[] lnames, String key) {
        System.out.println("Name                 | Position          | Separation Date");
        System.out.println("---------------------|-------------------|----------------");
        for (String s : employees.keySet()) {
            Employee e = employees.get(s);
            if (s.contains(key) || e.fname.contains(key)) {
                System.out.printf("%-20s | %-15s | %-20s\n", e.fname + " " + e.lname, e.position, e.date);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        filter(employees, lnames, in.nextLine());
    }
}